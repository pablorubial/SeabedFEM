...
function compute_scat_coefs(N, params::EndFireParams)
    ...
    Βsel_F(k_F, k) = k_F > abs(k) ? 1im*sqrt(k_F^2 - k^2) : 1im*sqrt(k^2 - k_F^2)
    Βsel_P(k_P, k) = abs(k_P) > abs(k) ? -1im*sqrt(k_P^2 - k^2) : -1im*sqrt(k^2 - k_P^2)
...
#     ux = 1 / (params.ρ_F * params.ω^2) * sum(πF_s[j] * 1im * k[j] * 1/h_fft *  exp(βF[j] * (x[2] - params.t_P)) * exp(1im * k[j] * (x[1] + params.L/2 + params.d_PML)/h_fft) for j in eachindex(βF))
...
function uF_s(x, βF, πF_s, k, h_fft, params::EndFireParams)
    ...
    ux = 1 / (params.ρ_F * params.ω^2) * sum(πF_s[j] * 1im * k[j] * 1 *  exp(βF[j] * (x[2] - params.t_P)) * exp(1im * k[j] * (x[1] + params.L/2 + params.d_PML)) for j in eachindex(βF))
...
#     ux = 1 / (params.ρ_P * params.ω^2) * sum(πP[j] * 1im * k[j] * 1/h_fft * exp(βP[j] * (x[2] - params.t_P)) * exp(1im * k[j] * (x[1] + params.L/2 + params.d_PML)/h_fft) for j in eachindex(βP))
...
function uP(x, βP, πP, k, h_fft, params::EndFireParams)
    ...
    ux = 1 / (params.ρ_P * params.ω^2) * sum(πP[j] * 1im * k[j] * 1 * exp(βP[j] * (x[2] - params.t_P)) * exp(1im * k[j] * (x[1] + params.L/2 + params.d_PML)) for j in eachindex(βP))
...
function u_incident_modes(x, πF_s, πP, βF, βP, k, h_fft, params::EndFireParams)    
    ...
    elseif abs(x[1]) <= params.L/2 && (x[2] - params.t_P) >= 0 && (x[2] - (params.t_P + params.t_F)) <= 1e-1
