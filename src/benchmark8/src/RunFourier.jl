...
N = 20 # Number of points to discretize the interface domain [-]
...
u_inc(x) = u_inc_F_controlled(x, params)
...
# writevtk(Ω,"./results/fourier_darcy.vtu", cellfields=[ "Re(u_scat)"=>real(u_scat), "Im(u_scat)"=>imag(u_scat)])  
