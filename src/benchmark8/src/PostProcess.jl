...
u_incident(x) =  uF_incident_controlled(x, params)
Π_incident(x) = πF_incident_controlled(x, params)
...
# Contruct now, the fourier solution
...
u_incident_fourier(x) = u_incident_modes(x, πF_s, πP, βF, βP, k, params)
u_inc_fourier(x) = u_inc_F_controlled(x, params)
